...
void addNode()
{
	...
	{ // step 2: insert the new node at the beginning
		...
		if (START != NULL)
		{
			START->prev = newNode; // step 4: make the first node point to NULL
		}
		newNode->prev = NULL; // step 5: make  the new node point to NULL
		START = newNode;	// step 6: mak ethe new node the first node
	}else
	{
...
void deleteNode()
{
	...
	if (current == START)
	{
		START = START->next; // step 2: update the SATRT pointer
		if (START != NULL)
		{
...
int main()
{
	while (true)
	{
		try
		{
			...
			cout << "3. View alll records in the ascending order of roll numbers" << endl;
